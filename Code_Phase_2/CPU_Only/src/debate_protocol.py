...
# C1R (Experiment E1): the focal re-answers with the SAME Round-1 framing and
# token budget as the debate conditions, but sees NO peer messages — only its
# own Round-0 answer. Isolates "second-chance reasoning" from peer exposure.
SOLO_REANSWER_INSTRUCTION = (
    "Review your previous response above. Reconsider the problem carefully and "
    "state your final answer with reasoning. You may keep or revise your answer. "
    "End with:\n"
    "Final answer: <your answer>\n"
    "Confidence: <integer from 0 to 100 representing how confident you are in your final answer>"
)
...
def build_solo_reanswer_prompt(
    question_text: str,
    answer_options,
    own_round0_text: str,
) -> Tuple[str, str]:
    """Build the C1R Round-1 prompt: focal sees only its own Round-0 answer."""
...
def run_round1_solo_reanswer(
    ...
    """
    Execute Round 1 for C1R (solo re-answer). Only the focal agent acts; it sees
    its own Round-0 response and re-answers with NO peer messages.
    """
    answer_options_str = ""
    _ans_opts = question.get("answer_options")
    if _ans_opts and isinstance(_ans_opts, (str, list)):
    ...
    focal_info = next((a for a in agents if a["role"] == "smart_focal"), agents[0])
    focal_id = focal_info["identifier"]
    own_r0 = next((r for r in round0_responses if r["agent_identifier"] == focal_id), None)
    own_r0_text = own_r0["raw_response_text"] if own_r0 else ""

    system, user = build_solo_reanswer_prompt(
        question_text=question["question_text"],
        answer_options=question.get("answer_options"),
        own_round0_text=own_r0_text,
    )

    agent_response = focal_info["agent"].generate_response(
        system_prompt=system,
        user_prompt=user,
        temperature=focal_info.get("temperature", 0.7),
        maximum_output_tokens=focal_info.get("max_tokens", 600),
        request_metadata={"question_id": question["question_identifier"], "round": 1, "condition": "C1R"},
    )
    ...
    return [{
        "agent_identifier": focal_id,
        "role": "smart_focal",
        "model_name": agent_response.model_name_returned_by_provider,
        "provider": focal_info.get("provider", "unknown"),
        "raw_response_text": agent_response.raw_text_output,
        "extracted_final_answer": answer,
        "extracted_self_reported_confidence_integer": confidence,
        "confidence_parse_status": conf_status,
        "answer_extraction_method": method,
        "peer_messages_seen_in_context": None,   # by construction: no peers
