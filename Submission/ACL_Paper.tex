...
At $n = 1{,}500$, the design cannot distinguish a debate-driven
revision benefit from multi-sample aggregation; pairwise $\kappa$ on
the three C2 agents is $0.72$
(Appendix~\ref{ssec:dependence_app}), corresponding to
$\mathrm{ESS} \approx 1.2$ effective draws.

C4 is qualitatively different.
The $+7.9$~pp trial-level gain (and $+7.0$~pp at question level) is
driven by an I$\rightarrow$C flip rate jumping from $27.5\%$ in C2 to
$55.8\%$ in C4: weak peers' Round-0 answers carry enough signal about
the focal's missed questions to roughly double the I$\rightarrow$C
rate, even when many peer answers are wrong.
Paired McNemar tests at trial and question levels agree on the
direction: at the question level, $33$ of $300$ items move from
incorrect (C1) to correct (C4) against $12$ moving the other way
($\chi^2 = 8.9$, Holm-corrected $p = 0.017$); at the trial level,
$187$ of $1{,}500$ trials improve against $68$ that regress
($p \ll 0.001$).
The question-level gain is larger on probe-hard items
($+14$ of $152$) than on probe-easy items ($+7$ of $148$),
consistent with the correction-signal interpretation.
Decomposing the C4 result into its two flip components: C$\rightarrow$I
rises only modestly from $5.0\%$ to $6.5\%$ (Cohen's $h = 0.05$,
$p = 0.15$, power $0.30$---underpowered, suggestive only), whereas
I$\rightarrow$C rises from $27.5\%$ to $55.8\%$ ($h = 0.58$,
$p < 0.001$, power $> 0.99$); the C4 net gain is dominated by the
correction signal.
...
across C2--C4 ($n = 4{,}500$ trials in 300 question clusters) yields
an odds ratio of $1.19$ ($[1.08, 1.31]$, $p = 6 \times 10^{-4}$) for
\textit{DeepSeek-v4-flash}: each weak peer multiplies the odds of a
correct Round-1 answer by $19\%$, the I$\rightarrow$C correction
mechanism dominating.
For \textit{GPT-4o-mini}, the same regression gives
$\mathrm{OR} = 0.97$ ($[0.80, 1.16]$, $p = 0.71$, $n = 750$): the
point estimate is below one but the cross-validation sample is too
small to establish a dose-response.
...
In C4, the focal Round-1 answer does not match either peer's
Round-0 answer in $98.0\%$ of GSM8K trials and $95.6\%$ of MMLU-Pro
trials (the focal produces a fresh answer rather than echoing a
peer); within these fresh-recompute trials, focal accuracy is
$94.5\%$ on GSM8K versus $83.6\%$ on MMLU-Pro---arithmetic
recomputation yields ground truth on math whereas MCQ leaves room to
land on a wrong option.
...
\citet{du2023improving} report debate gains under homogeneous peers
driven by aggregation of correct arguments;
\citet{koo2024benchmarking,sharma2023towards} document conformity and
sycophancy under single-agent wrong-position prompting.
The natural prediction is that confidently-wrong peer messages
degrade focal accuracy.
The opposite holds here for the stronger focal model: the
I$\rightarrow$C signal from wrong-anchored peer Round-0 messages
($55.8\%$ in C4) outweighs the C$\rightarrow$I conformity signal
($6.5\%$).
The mechanism is not generic ``peer wisdom'': it operates via
arithmetic recomputation on GSM8K
(Section~\ref{sub:crossmodel}), so wrong peers cue the focal toward
its R0-incorrect questions where fresh recomputation can land on the
correct answer.

\textit{GPT-4o-mini} fits the prior literature more conventionally:
its C$\rightarrow$I rate of $17.2\%$ in C4 is roughly one in six of
its R0-correct trials and the net accuracy effect is null.
The divergence is consistent with a focal-strength-dependent
threshold, but $n = 2$ focal models is too narrow to establish one.

On mitigation, confidence-weighted peer filtering is unavailable
here: a filter on either round lacks usable signal
(Section~\ref{sub:protocol},
Appendix~\ref{ssec:dependence_app}), so mitigations relying on
externally verified peer accuracy---held-out calibration sets or
architecturally distinct verifier models---are required instead.
...
peers ($p = 0.017$ Holm-corrected), concentrated on GSM8K
(interaction $\mathrm{OR} = 1.61$) and driven by fresh arithmetic
recomputation; \textit{GPT-4o-mini}'s rate of flipping a correct
answer to an incorrect one nearly triples ($5.8\%$ to $17.2\%$)
without a detectable accuracy change.
The homogeneous-debate condition's lift over solo is not detectable
above multi-sample aggregation, and a confidence-weighted peer
filter strips all peer messages because weak peers do not produce a
usable confidence signal; the originally specified pre-flight gate
is misspecified, and a corrected gate predicts the filter failure.
