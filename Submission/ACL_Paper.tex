...
On mitigation, confidence-weighted peer filtering is unavailable: a
filter on either round lacks usable signal (weak agents emit no
Round-0 confidence integer; Round-1 distributions are
non-discriminative, Section~\ref{sub:protocol},
Appendix~\ref{ssec:dependence_app}).
Mitigations depending on externally verified peer accuracy---held-out
calibration sets or architecturally distinct verifier models---are
required instead.
...
$+7.9$~pp in C4 (Holm $p = 0.017$), concentrated on GSM8K
($\mathrm{OR}_{\text{int.}} = 1.61$, $p \approx 10^{-5}$) and driven
by fresh arithmetic recomputation; \textit{GPT-4o-mini}'s
C$\rightarrow$I rate triples ($5.8\%\!\to\!17.2\%$) without a
detectable accuracy effect.
C2's homogeneous-debate lift is not detectable above multi-sample
aggregation (SC-3, $p = 0.22$).
The C5 filter strips all peer messages; the originally specified
calibration gate is misspecified, and a corrected gate based on the
discriminative gap predicts the C5 null deterministically.
