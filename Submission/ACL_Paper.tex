...
\begin{abstract}
Multi-agent debate among large language models (LLMs) can improve
aggregate reasoning accuracy.
Whether this benefit holds when agents differ markedly in capability
has not been tested at scale.
Under capability asymmetry, a strong focal model receives peer reasoning
of unknown quality, with potentially opposing effects on accuracy and
answer stability.
This study examines $7{,}300$ trials on $300$ questions from MMLU-Pro
and GSM8K, testing two focal models against systematically varied numbers
of confidently-wrong peer agents. \textit{DeepSeek-v4-flash} gains $7.9$ percentage points post-debate on
GSM8K arithmetic but shows no reliable accuracy change on MMLU-Pro
multiple-choice items once peer source is controlled.
\textit{GPT-4o-mini} shows no net accuracy change; its correct-to-incorrect
flip rate nearly triples, from $5.8\%$ to $17.2\%$, under two wrong peers. Two structural findings extend these results.
Homogeneous debate is statistically indistinguishable from averaging three
independent samples of the same model; the homogeneous condition therefore
functions as a sampling control rather than a genuine debate test.
A confidence-weighted peer filter, proposed for mixed-capability ensembles,
fails here entirely because wrong-peer personas suppress the confidence signal.
The standard pre-flight test for this filter passes despite the failure;
a corrected formulation is proposed that detects filter failure before deployment.
\end{abstract}
...
Table~\ref{tab:conditions} summarises accuracy and flip-rate outcomes across all five conditions and both focal agents.

%% ── 4.1 ──────────────────────────────────────────────────────────────────
\subsection{Accuracy and flip rates across conditions}
\label{sub:accuracy}

For \textit{DeepSeek-v4-flash}, Round-1 accuracy reaches $84.1\%$ in C4, a gain of $+7.9$~pp over the $76.2\%$ solo baseline ($95\%$ CI $[+5.1, +10.8]$~pp, $p < 10^{-4}$); the C2 ($78.7\%$) and C3 ($78.3\%$) gains are not detectable at $n = 1{,}500$ trials per condition.

A self-consistency-of-three baseline (SC3: per-trial majority vote of the three C2 Round-0 answers before debate exchange) attains $76.8\%$ accuracy; the $1.9$~pp gap between C2's Round-1 ($78.7\%$) and SC3 is not detectable ($p = 0.22$).
Pairwise $\kappa$ on the three smart agents' Round-0 correctness is $0.72$ (Appendix~\ref{ssec:dependence_app}), indicating near-homogeneous draws; the design cannot distinguish a debate-driven revision benefit from multi-sample aggregation at $n = 1{,}500$.

C4 is qualitatively different.
The $+7.9$~pp gain is driven by the I$\rightarrow$C flip rate jumping from $27.5\%$ in C2 to $55.8\%$ in C4, while C$\rightarrow$I rises only modestly ($5.0\%$ to $6.5\%$; underpowered, $p = 0.15$).
At the question level, $33$ of $300$ items move correct in C4 that were incorrect in C1, against $12$ moving the other way (Holm-corrected $p = 0.017$): the net C4 gain is dominated by the I$\rightarrow$C correction signal.

For \textit{GPT-4o-mini}, net Round-1 accuracy is null at the cross-validation sample size ($n = 250$; both C3 and C4 contrasts have CIs overlapping zero).
The C$\rightarrow$I flip rate rises clearly from $5.8\%$ in C2 to $17.2\%$ in C4 ($p = 0.001$, post-hoc power $0.91$): the two models diverge on flip dynamics, with \textit{GPT-4o-mini} showing a roughly three-fold increase in conformity-revision pressure whereas \textit{DeepSeek-v4-flash}'s C$\rightarrow$I rate stays within $[5.0\%, 6.5\%]$ across C2--C4.
C5 yields accuracy $1.6$~pp below C4 on the matched 100-question subset ($p = 0.453$ McNemar, $n = 300$ trials): every peer message is excluded by the Round-0 confidence filter in $100\%$ of trials, rendering C5 functionally a solo condition; the protocol as configured strips its own input.

\begin{table*}[t]
\centering
\small
\caption{Per-condition accuracy and flip rates. $\Delta$ = Round-1 vs.\ C1 solo; C$\to$I = harmful-flip rate; I$\to$C = correction rate; S = smart; D = weak (dumb) agent.\label{tab:conditions}}
...
\par\smallskip
{\footnotesize
$^{\dagger}$C5: 100-question stratified subset $\times$ 3 replications ($N=300$); $\Delta$ = C5 vs.\ C4 on the matched subset, not C5 vs.\ C1.\\
$^{\ddagger}$SC3: per-trial majority vote of the three C2 Round-0 answers before debate exchange; isolates debate revision from multi-sample aggregation.
}
\end{table*}
...
\begin{figure}[t]
  ...
  \caption{Round-1 accuracy vs.\ weak-peer count. Blue: \textit{DeepSeek-v4-flash} ($N{=}1{,}500$, 95\% CI); red: \textit{GPT-4o-mini} ($N{=}250$); dotted: C1 baselines.\label{fig:doseresponse}}
\end{figure}
...
Direct numerical comparison with single-agent conformity benchmarks (CoBBLEr~\citep{koo2024benchmarking}, BenchForm~\citep{weng2025benchform}, SYCON-Bench~\citep{hong2025sycon}) is not straightforward: these benchmarks use honest peers or single-agent prompts, not adversarial wrong-anchored personas.
The within-study peer-count gradient is the primary quantitative contribution; cross-benchmark harmonisation is deferred to future work.
...
statistically null. Weak-peer messages on numeric word problems supply the focal agent with alternative working that can be re-checked arithmetically, while ten-option multiple choice gives a smaller margin for the same mechanism.
The C$\rightarrow$I flip rate is $6.4\%$ on MMLU-Pro items but only $1.8\%$ on GSM8K items; subject-level variation within MMLU-Pro is wide, with physics and law above $20\%$ and biology below $4\%$ (Appendix~S8, with the multiple-testing caveat noted there).
...
All experimental artefacts---per-trial log (\texttt{trial\_log.parquet}, 35{,}050~rows), consolidated answers (\texttt{final\_answers.parquet}, 7{,}300~rows), the question pool, persona files, and two analysis scripts that reproduce every reported number---will be released as an anonymised repository for review and deposited at Zenodo on acceptance.
Full model snapshots, random seeds, answer-extraction details, and compute cost are in Appendix~\ref{ssec:repro_app}.
...
\begin{figure}[h]
  ...
  \caption{C$\to$I flip rate by MMLU-Pro subject and GSM8K in C4 (\textit{DeepSeek-v4-flash}); error bars: 95\% bootstrap CI.\label{sfig:subjects}}
\end{figure}
