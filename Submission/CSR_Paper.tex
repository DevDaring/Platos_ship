...
another, often improving aggregate performance on reasoning tasks.
When agents differ in capability, peer exposure exerts two simultaneous
forces on a strong focal agent: a correction signal from peers who got
different questions right, and a corruption signal from peers who hold
incorrect positions.
This work presents a controlled five-condition experiment that varies the
number of weak peers exposed to a strong focal agent, using
\textit{DeepSeek-v4-flash} as the primary focal model and
\textit{GPT-4o-mini} for cross-model comparison, across 300
\textit{MMLU-Pro} and \textit{GSM8K} questions with five replications
per question.
For \textit{DeepSeek-v4-flash} the correction signal dominates:
Round~1 accuracy rises from 76.2\% solo to 84.1\% with two weak peers
($+$7.9~pp).
For \textit{GPT-4o-mini} the corruption signal dominates: accuracy falls
4.0~pp under one weak peer, with a correct-to-incorrect flip rate of
17.2\% under two.
Confidence-weighted peer filtering fails as a mitigation because weak
agents over-report certainty in 99.5\% of trials.
Debate protocols should weight peers by verifiable accuracy, not
...
per question, covering 8{,}500 trials across five conditions.
...
\begin{enumerate}
    \item A controlled three-point peer-count gradient measurement of
          focal-agent accuracy and correct-to-incorrect flip rate in a
          strong LLM reasoner exposed to up to two weak peers.
    \item A bias-theoretic decomposition into correction (I\textrightarrow{}C)
          and corruption (C\textrightarrow{}I) signals, with an explicit
          design-limit discussion of bandwagon attribution under
          unanimous-wrong peer conditions.
    \item A cross-model comparison with \textit{GPT-4o-mini} showing that
          the corruption signal dominates in a weaker focal model, and an
          empirical confirmation that confidence-weighted peer filtering fails
          when weak agents over-report certainty.
\end{enumerate}
...
\textit{MMLU-Pro}~\citep{wang2024mmlupro} contributes 250 questions spanning
ten subject categories (biology, business, chemistry, computer science,
economics, engineering, health, history, law, and psychology).
\textit{GSM8K}~\citep{cobbe2021training} contributes 50 mathematical word
problems.
The resulting 300-question pool is answered five times independently per
condition, yielding 1,500 trials per condition for the primary experiment
and 250 trials per condition for the cross-validation.
Each agent in conditions C2--C5 adopts one of four rhetorically distinct
personas (analytical, sceptical, diplomatic, assertive); details of the
generation and validation pipeline are given in Supplementary~S5.
...
A controlled five-condition experiment with 8{,}500 debate trials isolates
the effect of increasing weak-peer count on a strong focal agent's reasoning.
For \textit{DeepSeek-v4-flash}, the net accuracy gain in C4 masks a
significant question-level regression ($p = 0.0025$, McNemar;
Bonferroni-corrected $p = 0.017$) and a monotone rise in the
correct-to-incorrect flip rate from 5.0\% to 6.5\% across conditions.
For \textit{GPT-4o-mini}, the degradation is unambiguous: accuracy falls
4.0 percentage points in C3 and the C$\rightarrow$I flip rate reaches
17.2\% in C4, nearly three times the value observed in the stronger model.
The sycophancy and bandwagon mechanisms documented for single-agent settings
therefore transfer to multi-agent debate when capability asymmetry is large.
C5 confirms that weak agents over-report certainty in 99.5\% of trials;
confidence-weighted peer filtering is better described as a demonstration
of weak-agent miscalibration than as a failed mitigation design.

These findings limit the conditions under which mixed-capability debate
can be recommended.
Debate groups that include agents with substantially lower baseline accuracy
should not rely on self-reported confidence as a proxy for peer quality.
Future work should evaluate whether architecturally distinct judge models
or externally verified calibration records can provide the capability signal
that self-report cannot, and should extend the three-point peer-count
gradient measurement to open-ended tasks where binary correctness is
not available.
