#!/usr/bin/env bash
# autopush_vast.sh — push results and logs to GitHub every 30 minutes.
# Runs ON the Vast.ai instance, which bills by the second and can be
# reclaimed, so nothing may live only on its disk.
#
...
BRANCH="${BRANCH:-main}"
WORKDIR="${WORKDIR:-/workspace/platos_push}"
RESULTS_DIR="${RESULTS_DIR:-/workspace/platos/results}"
LOG_DIR="${LOG_DIR:-/workspace/platos/logs}"
REMOTE_SUBDIR="${REMOTE_SUBDIR:-Code/Code_Phase_3/Vast_AI_Big_Model_Run/results}"
...
push_once() {
  ...
  for extra in big_probe.log smoke.log install.log run.out; do
    cp -f "/workspace/platos/$extra" "$WORKDIR/$REMOTE_SUBDIR/logs/" 2>/dev/null || true
  done
...
p = "/workspace/platos/results/big_probe_trials.parquet"
d = pd.read_parquet(p)
