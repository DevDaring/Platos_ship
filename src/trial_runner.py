...
from .debate_protocol import run_round0, run_round1_standard
...
class TrialRunner:
    ...
    def _build_agents_for_condition(
        ...
        """Build agent list for a specific condition."""
        cond_config = self.exp_config["main_conditions"].get(condition_id)
        if not cond_config:
            cond_config = self.exp_config["mitigation_condition"].get(condition_id, {})
        ...
        focal_agent = self.smart_agents[focal_agent_name]
        ...
        for i in range(smart_count):
            agents.append({
                ...
                "agent": focal_agent,
                "model_name": getattr(focal_agent, "model_name", "unknown"),
                "provider": focal_agent.provider,
                "temperature": 0.7,
                "max_tokens": 600,
            })
        ...
        if dumb_count > 0:
            ...
            if dumb_count == 1:
                # C3: random choice of Llama or Gemma
                chosen = rng.choice(dumb_agent_keys)
                dumb_agent = self.dumb_agents[chosen]

                # Get persona
                persona = self._get_persona(question["question_identifier"], trial_index, 0, rng)

                agents.append({
                    "identifier": "Agent_Dumb_1",
                    "role": "dumb",
                    "agent": dumb_agent,
                    "model_name": getattr(dumb_agent, "actually_loaded_repository", None) or getattr(dumb_agent, "model_name", chosen),
                    "provider": getattr(dumb_agent, "provider", "local_huggingface"),
                    "persona_text": persona,
                    "temperature": 0.9,
                    "max_tokens": 350,
                })
            elif dumb_count >= 2:
                # C4/C5: one Llama + one Gemma
                for j, key in enumerate(dumb_agent_keys[:2]):
                    dumb_agent = self.dumb_agents[key]
                    persona = self._get_persona(question["question_identifier"], trial_index, j, rng)

                    agents.append({
                        "identifier": f"Agent_Dumb_{j+1}",
                        "role": "dumb",
                        "agent": dumb_agent,
                        "model_name": getattr(dumb_agent, "actually_loaded_repository", None) or getattr(dumb_agent, "model_name", key),
                        "provider": getattr(dumb_agent, "provider", "local_huggingface"),
                        "persona_text": persona,
                        "temperature": 0.9,
                        "max_tokens": 350,
    ...
    def _get_persona(self, question_id: str, trial_index: int, dumb_index: int, rng) -> Optional[str]:
        """Get a persona variant for a question."""
        q_personas = self.personas_df[
            (self.personas_df["question_identifier"] == question_id) &
            (self.personas_df["validation_pass_status"] == "passed")
        ]
        if q_personas.empty:
            q_personas = self.personas_df[self.personas_df["question_identifier"] == question_id]

        if q_personas.empty:
            return None

        # Use trial index to select variant
